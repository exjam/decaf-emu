...
bool CodeCache::initialise(size_t codeSize, size_t dataSize)
{
   mReserveAddress = 0;
...
void CodeCache::clear()
{
   ...
   for (auto offset = 0u; offset < mDataAllocator.allocated; offset += sizeof(CodeBlock)) {
      ...
      RtlDeleteFunctionTable(&block->rtlFuncTable);
   }
#endif
      ...
      for (auto i = 0u; i < Level1Size; ++i) {
         ...
         for (auto j = 0u; level2 && j < Level2Size; ++j) {
            ...
            for (auto k = 0u; level3 && k < Level3Size; ++k) {
               level3[j].store(BlockUncompiled);
            }
...
size_t CodeCache::getCodeCacheSize()
{
   return mCodeAllocator.allocated;
}

void CodeCache::setBlockIndex(uint32_t address, CodeBlockIndex index)
{
   decaf_check(index >= 0);
   getIndexPointer(address)->store(index);
}

CodeBlock *CodeCache::getBlockByAddress(uint32_t address)
{
...
CodeBlock *CodeCache::getBlockByIndex(CodeBlockIndex index)
{
   auto blockAddress = mDataAllocator.baseAddress + index * sizeof(CodeBlock);
   return reinterpret_cast<CodeBlock *>(blockAddress);
}

std::atomic<CodeBlockIndex> *CodeCache::getIndexPointer(uint32_t address)
{
   ...
   if (UNLIKELY(!level3)) {
      ...
      std::memset(newLevel3, BlockUncompiled, sizeof(newLevel3[0]) * Level3Size);
...
CodeBlockIndex CodeCache::getIndex(uint32_t address)
{
   return getIndexPointer(address)->load();
}

CodeBlockIndex CodeCache::getIndex(CodeBlock *block)
{
...
CodeBlock *CodeCache::registerCodeBlock(uint32_t address, void *code, size_t size, void *unwindInfo, size_t unwindSize)
{
   ...
   decaf_check(unwindSize <= CodeBlock::MaxUnwindInfoSize);
   block->unwindSize = static_cast<uint32_t>(unwindSize);
   std::memcpy(block->unwindInfo.data(), unwindInfo, unwindSize);

   auto unwindAddress = reinterpret_cast<uintptr_t>(block->unwindInfo.data());
   block->rtlFuncTable.BeginAddress = static_cast<DWORD>(codeAddress - mReserveAddress);
   block->rtlFuncTable.EndAddress = static_cast<DWORD>(block->rtlFuncTable.BeginAddress + size);
   block->rtlFuncTable.UnwindData = static_cast<DWORD>(unwindAddress - mReserveAddress);
   RtlAddFunctionTable(&block->rtlFuncTable, 1, mReserveAddress);
#endif
...
gsl::span<CodeBlock> CodeCache::getCompiledCodeBlocks()
{
...
uintptr_t CodeCache::allocate(FrameAllocator &allocator, size_t size, size_t alignment)
{
