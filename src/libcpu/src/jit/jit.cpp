...
struct JitProfileStats
{
   uint64_t totalCodeSize;
   std::atomic<uint64_t> totalProfileTime;
};
...
static size_t
sCacheCodeSize = 1024 * 1024 * 1024;

// Maximum size of JIT data cache
static size_t
sCacheDataSize = 512 * 1024 * 1024;
...
getCodeBlock(Core *core, ppcaddr_t address);
...
{
   return getCodeBlock(core, address)->code;
}
...
static NEVER_INLINE CodeBlock *
getCodeBlock(Core *core, ppcaddr_t address)
{
   auto indexPtr = sCodeCache.getIndexPointer(address);
   auto blockIndex = indexPtr->load();

   // If block is uncompiled, let's try mark it as compiling!
   if (UNLIKELY(blockIndex == CodeCache::BlockUncompiled)) {
      if (!indexPtr->compare_exchange_strong(blockIndex, CodeCache::BlockCompiling)) {
         // Another thread has started compiling, wait for it to finish.
         while (blockIndex == CodeCache::BlockCompiling) {
   ...
   if (LIKELY(blockIndex >= 0)) {
      auto block = sCodeCache.getBlockByIndex(blockIndex);
      return block;
   }
   ...
   if (data && data->id == espresso::InstructionID::b && !instr.lk) {
      ...
      if (target != address) {
         auto block = sCodeCache.getBlockByAddress(target);

         if (block) {
            sCodeCache.setBlockIndex(address, sCodeCache.getIndex(block));
            return block;
         }
      }
   }
   ...
   auto block = sCodeCache.registerCodeBlock(address, code, size, unwindInfo, unwindSize);
   decaf_check(block);
   free(buffer);
...
{
   sCacheCodeSize = cacheSize;
   sCodeCache.initialise(sCacheCodeSize, sCacheDataSize);
}
...
resumeWithVerify()
{
   ...
   do {
      ...
      auto codeBlock = getCodeBlock(core, core->nia);
      ...
      if (codeBlock) {
         ...
         auto entry = reinterpret_cast<JitEntry>(codeBlock->code);
         entry(core, mem::base());
      } else {
...
resume()
{
   ...
   do {
      ...
      if (1) {
         ...
         if (LIKELY(block)) {
            auto entry = reinterpret_cast<JitEntry>(block->code);
            entry(core, memBase);
         } else {
...
getJitCodeSize()
{
   return jit::sCodeCache.getCodeCacheSize();
}
...
{
   return jit::sCodeCache.getCompiledCodeBlocks();
}

uint64_t
getJitTotalProfileTime()
{
   return jit::sTotalProfileTime.load();
}
...
resetJitProfileData()
{
   ...
   auto blocks = jit::sCodeCache.getCompiledCodeBlocks();

   for (auto &block : blocks) {
      block.profileData.count.store(0);
      block.profileData.time.store(0);
   }
}
