...
processEntryPoint(phys_ptr<void> /* context */)
{
   ...
   internal::initialiseStaticMainMetaServerData();
   ...
   // Start MetaServer for /dev/acp_main
   error = internal::startMainMetaServer();
   if (error < Error::OK) {
      internal::acpLog->error(
         "processEntryPoint: startMainMetaServer failed with error = {}", error);
      return error;
   }
