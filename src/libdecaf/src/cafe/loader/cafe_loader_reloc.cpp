                  ...
                  virt_ptr<char> name)
{
   ...
   auto begin = 0u;
   auto end = numExports - 1;

   while (begin <= end) {
      auto pos = (begin + end) / 2;
      auto exportName = strTable + (exports[pos].name & 0x7FFFFFFF);
      auto cmp = strncmp(name.getRawPointer(),
                         exportName.getRawPointer(),
                         0xFFFF);
      if (cmp == 0) {
         return virt_addrof(exports[pos]);
      } else if (cmp < 0) {
         begin = pos + 1;
      } else if (cmp > 0) {
         end = pos + 1;
                     ...
                     uint32_t unk)
{
   ...
   for (auto i = 1u; i < numSymbols; ++i) {
      ...
      auto symbolExport = LiBinSearchExport(imports[symbol->shndx].exports,
                                            imports[symbol->shndx].numExports,
                                            symbolName);
      if (!symbolExport) {
               ...
               uint32_t fileType)
{
   ...
   case rpl::R_PPC_REL24:
   {
      ...
      *virt_cast<uint32_t *>(target) =
         (*virt_cast<uint32_t *>(target) & 0x3FFFFFC) |
         (static_cast<uint32_t>(distance) & 0xFC000003);
      break;
   }
   default:
                   ...
                   uint32_t unk)
{
   ...
   rpl->loadStateFlags &= ~LoaderStateFlagBit2;
   return 0;
}
