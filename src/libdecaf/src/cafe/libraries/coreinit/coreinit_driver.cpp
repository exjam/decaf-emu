                  ...
                  virt_ptr<BOOL> outDidInit)
{
   ...
   if (dynloadError != OSDynLoad_Error::OK) {
      COSWarn(COSReportModule::Unknown1,
   ...
   OSUninterruptibleSpinLock_Acquire(virt_addrof(driverData->lock));

   // Check if a driver has already been registered with the same name
   for (auto other = driverData->registeredDrivers; other; other = other->next) {
      ...
      if (!stricmp(otherName.getRawPointer(), name.getRawPointer())) {
         ...
         OSUninterruptibleSpinLock_Release(virt_addrof(driverData->lock));
         goto error;
      }
   }
   ...
   if (dynloadError == OSDynLoad_Error::OK) {
      ...
      if (driver->unk0x10 < driverData->minUnk0x10) {
         driverData->minUnk0x10 = driver->unk0x10;
      }

      if (driver->unk0x10 > driverData->maxUnk0x10) {
         driverData->maxUnk0x10 = driver->unk0x10;
      }

      // Insert to front of list
      driver->next = driverData->registeredDrivers;
      driverData->registeredDrivers = driver;
   ...
   for (auto i = 0u; i < driver->interfaceModuleHandles.size(); ++i) {
      if (driver->interfaceModuleHandles[i] == moduleHandle) {
         ...
         driver->interfaceModuleHandles[i] = 0;
      }
   }
                    ...
                    OSDriver_UserDriverId userDriverId)
{
   ...
   auto driver = driverData->registeredDrivers;
   while (driver) {
   ...
   if (driverData->ationCallbackDriver == driver) {
      COSWarn(COSReportModule::Unknown1,
              "***OSDriver_Deregister() of self from inside action callback!\n");
      driverData->ationCallbackDriver = nullptr;
   }
   ...
   if (prev) {
      ...
      driverData->registeredDrivers = driver->next;
   }
