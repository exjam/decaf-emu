...
{
   be2_val<FSCmdQueueHandlerFn> handleDequeuedCommand;
   be2_val<IOSAsyncCallbackFn> handleFsaAsyncCallback;
};

static virt_ptr<StaticFsClientData>
getFsClientData()
{
   return Library::getStaticData()->fsClientData;
}
              ...
              FSErrorFlag errorMask)
{
   ...
   internal::fsCmdQueueCreate(virt_addrof(clientBody->cmdQueue),
                              getFsClientData()->handleDequeuedCommand, 1);
                          ...
                          FSErrorFlag errorMask)
{
   ...
   if (result >= 0) {
      ...
      OSReceiveMessage(virt_addrof(blockBody->syncQueue), message, OSMessageFlags::Blocking);
                               ...
                               virt_ptr<void>context)
{
   ...
   blockBody->fsaStatus = FSAShimDecodeIosErrorToFsaStatus(clientBody->clientHandle,
                                                           error);
      ...
      */
      ...
      if (!fsInitialised() || fsDriverDone()) {
         ...
         error = fsaShimSubmitRequestAsync(&blockBody->fsaShimBuffer,
                                           ...
                                           getFsClientData()->handleFsaAsyncCallback,
                                           blockBody);
      }
...
void
Library::initialiseFsClientStaticData()
{
   auto fsData = allocStaticData<StaticFsData>();
   getStaticData()->fsData = fsData;

   fsData->handleDequeuedCommand = GetFunctionAddress(internal::fsClientHandleDequeuedCommand);
   fsData->handleFsaAsyncCallback = GetFunctionAddress(internal::fsClientHandleFsaAsyncCallback);
}

void
Library::registerFsClientFunctions()
{
   ...
   RegisterFunctionInternal(internal::fsClientHandleDequeuedCommand);
   RegisterFunctionInternal(internal::fsClientHandleFsaAsyncCallback);
}
