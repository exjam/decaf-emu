...
getLockedCacheData()
{
   return Library::getStaticData()->lockedCacheData;
}

virt_ptr<LockedCacheState>
getCoreLockedCacheState()
{
   return virt_addrof(getLockedCacheData()->coreState[OSGetCoreId()]);
}
...
LCDealloc(virt_ptr<void> ptr)
{
   ...
   auto index = (addr - lcState->baseAddress) / LCBlockSize;
   ...
   lcState->allocatedMasks[index] = 0;
   lcState->allocatedSize[index] = 0;
...
void
Library::initialiseLockedCacheStaticData()
{
   auto lcData = allocStaticData<StaticLockedCacheData>();
   getStaticData()->lockedCacheData = lcData;
}

void
Library::registerLockedCacheFunctions()
{
