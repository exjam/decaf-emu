...
static virt_ptr<StaticFsCmdData>
getFsCmdData()
{
   return Library::getStaticData()->fsCmdData;
}
                 ...
                 virt_ptr<const FSAsyncData> asyncData)
{
   ...
   internal::fsClientSubmitCommand(clientBody, blockBody, getFsCmdData()->finishCmd);
   return FSStatus::OK;
}
                          ...
                          virt_ptr<const FSAsyncData> asyncData)
{
   ...
   internal::fsClientSubmitCommand(clientBody,
                                   blockBody,
                                   getFsCmdData()->finishGetMountSourceNextOpenCmd);
   return FSStatus::OK;
}
                     ...
                     virt_ptr<const FSAsyncData> asyncData)
{
   ...
   auto error = internal::fsaShimPrepareRequestReadFile(virt_addrof(blockBody->fsaShimBuffer),
                                                        clientBody->clientHandle,
                                                        buffer,
                                                        1,
                    ...
                    virt_ptr<const FSAsyncData> asyncData)
{
   ...
   auto error = internal::fsaShimPrepareRequestGetInfoByQuery(virt_addrof(blockBody->fsaShimBuffer),
                                                              clientBody->clientHandle,
                                                              path, type);
                      ...
                      virt_ptr<const FSAsyncData> asyncData)
{
   ...
   auto error = internal::fsaShimPrepareRequestWriteFile(virt_addrof(blockBody->fsaShimBuffer),
                                                         clientBody->clientHandle,
                                                         buffer,
                                                         1,
...
void
Library::initialiseFsCmdStaticData()
{
   auto fsCmdData = allocStaticData<StaticFsCmdData>();
   getStaticData()->fsCmdData = fsCmdData;

   // These functions are registered in coreinit_fs_cmdblock.cpp
   fsCmdData->finishCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishCmd);
   fsCmdData->finishMountCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishMountCmd);
   fsCmdData->finishReadCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishReadCmd);
   fsCmdData->finishWriteCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishWriteCmd);
   fsCmdData->finishGetMountSourceNextOpenCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishGetMountSourceNextOpenCmd);
   fsCmdData->finishGetMountSourceNextReadCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishGetMountSourceNextReadCmd);
   fsCmdData->finishGetMountSourceNextCloseCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishGetMountSourceNextCloseCmd);
}

void
Library::registerFsCmdFunctions()
{
