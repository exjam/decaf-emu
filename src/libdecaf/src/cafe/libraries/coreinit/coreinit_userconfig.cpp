...
struct StaticUserConfigData
{
   ...
   be2_val<IOSAsyncCallbackFn> asyncCallbackFn = nullptr;
};

static virt_ptr<StaticUserConfigData>
staticUserConfigData()
{
   return Library::getStaticData()->userConfigData;
}
...
ucAllocateMessage(uint32_t size)
{
   ...
   auto userConfigData = staticUserConfigData();

   if (size == 0) {
      return nullptr;
   } else if (size <= SmallMessageSize) {
      message = IPCBufPoolAllocate(userConfigData->smallMessagePool, size);
   } else {
      message = IPCBufPoolAllocate(userConfigData->largeMessagePool, size);
   }
...
{
   auto userConfigData = staticUserConfigData();
   IPCBufPoolFree(userConfigData->smallMessagePool, message);
   IPCBufPoolFree(userConfigData->largeMessagePool, message);
}
...
{
   auto userConfigData = staticUserConfigData();
   OSLockMutex(virt_addrof(userConfigData->lock));

   if (!userConfigData->initialised) {
      if (!userConfigData->smallMessagePool) {
         userConfigData->smallMessagePool = IPCBufPoolCreate(virt_addrof(userConfigData->smallMessageBuffer),
                                                             static_cast<uint32_t>(userConfigData->smallMessageBuffer.size()),
                                                             SmallMessageSize,
                                                             virt_addrof(userConfigData->smallMessageCount),
                                                             1);
      }

      if (!userConfigData->largeMessagePool) {
         userConfigData->largeMessagePool = IPCBufPoolCreate(virt_addrof(userConfigData->largeMessageBuffer),
                                                             static_cast<uint32_t>(userConfigData->largeMessageBuffer.size()),
                                                             LargeMessageSize,
                                                             virt_addrof(userConfigData->largeMessageCount),
                                                             1);
      }

      if (userConfigData->smallMessagePool && userConfigData->largeMessagePool) {
         userConfigData->initialised = true;
      }
   }

   OSUnlockMutex(virt_addrof(userConfigData->lock));
                     ...
                     virt_ptr<UCAsyncParams> asyncParams)
{
   ...
   if (!asyncParams) {
      ...
      result = static_cast<UCError>(IOS_IoctlvAsync(handle,
                                                    ...
                                                    count + 1,
                                                    vecs,
                                                    staticUserConfigData()->asyncCallbackFn,
                                                    asyncParams));
   }
...
void
Library::initialiseUserConfigStaticData()
{
   auto userConfigData = allocStaticData<StaticUserConfigData>();
   getStaticData()->userConfigData = userConfigData;

   OSInitMutex(virt_addrof(userConfigData->lock));
   userConfigData->asyncCallbackFn = GetInternalFunctionAddress(internal::ucIosAsyncCallback);
}

void
Library::registerUserConfigFunctions()
{
   ...
   RegisterFunctionInternal(internal::ucIosAsyncCallback);
}
