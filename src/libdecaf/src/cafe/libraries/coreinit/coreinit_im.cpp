...
struct StaticImData
{
   ...
   be2_val<IOSAsyncCallbackFn> iosAsyncCallbackFn;
};

static virt_ptr<StaticImData>
getImData()
{
   return Library::getStaticData()->imData;
}
...
imAcquireItbMutex()
{
   OSLockMutex(virt_addrof(getImData()->itbMutex));
}

static void
imReleaseItbMutex()
{
   OSUnlockMutex(virt_addrof(getImData()->itbMutex));
}
...
IM_GetParameters(virt_ptr<IMParameters> parameters)
{
   ...
   result = IM_GetParameter(handle,
                            virt_addrof(imData->sharedRequest),
...
void
Library::initialiseImStaticData()
{
   auto imData = allocStaticData<StaticImData>();
   getStaticData()->imData = imData;

   OSInitMutex(virt_addrof(imData->itbMutex));
   imData->iosAsyncCallbackFn = GetInternalFunctionAddress(internal::imIosAsyncCallback);
}

void
Library::registerImExports()
{
   ...
   RegisterFunctionInternal(internal::imIosAsyncCallback);
}
