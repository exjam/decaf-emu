...
{
   be2_val<FSFinishCmdFn> finishCmd = nullptr;
   be2_val<FSFinishCmdFn> finishMountCmd = nullptr;
   be2_val<FSFinishCmdFn> finishReadCmd = nullptr;
   be2_val<FSFinishCmdFn> finishWriteCmd = nullptr;
   be2_val<FSFinishCmdFn> finishGetMountSourceNextOpenCmd = nullptr;
   be2_val<FSFinishCmdFn> finishGetMountSourceNextReadCmd = nullptr;
   be2_val<FSFinishCmdFn> finishGetMountSourceNextCloseCmd = nullptr;
};

static virt_ptr<StaticFsCmdBlockData>
getFsCmdBlockData()
{
   return Library::getStaticData()->fsCmdBlockData;
}
                        ...
                        FSStatus result)
{
   ...
   fsCmdBlockRequeue(virt_addrof(blockBody->clientBody->cmdQueue),
                     blockBody, FALSE,
                     getFsCmdBlockData()->finishReadCmd);
}
                         ...
                         FSStatus result)
{
   ...
   fsCmdBlockRequeue(virt_addrof(blockBody->clientBody->cmdQueue),
                     blockBody, FALSE,
                     getFsCmdBlockData()->finishWriteCmd);
}
                                          ...
                                          FSStatus result)
{
   ...
   fsCmdBlockRequeue(virt_addrof(blockBody->clientBody->cmdQueue),
                     blockBody, TRUE,
                     getFsCmdBlockData()->finishGetMountSourceNextReadCmd);
}
                                          ...
                                          FSStatus result)
{
   ...
   fsCmdBlockRequeue(virt_addrof(blockBody->clientBody->cmdQueue),
                     blockBody,
                     TRUE,
                     getFsCmdBlockData()->finishGetMountSourceNextCloseCmd);
}
...
void
Library::initialiseFsCmdBlockStaticData()
{
   auto fsCmdBlockData = allocStaticData<StaticFsCmdBlockData>();
   getStaticData()->fsCmdBlockData = fsCmdBlockData;

   fsCmdBlockData->finishCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishCmd);
   fsCmdBlockData->finishMountCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishMountCmd);
   fsCmdBlockData->finishReadCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishReadCmd);
   fsCmdBlockData->finishWriteCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishWriteCmd);
   fsCmdBlockData->finishGetMountSourceNextOpenCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishGetMountSourceNextOpenCmd);
   fsCmdBlockData->finishGetMountSourceNextReadCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishGetMountSourceNextReadCmd);
   fsCmdBlockData->finishGetMountSourceNextCloseCmd = GetInternalFunctionAddress(internal::fsCmdBlockFinishGetMountSourceNextCloseCmd);
}

void
Library::registerFsCmdBlockFunctions()
{
   ...
   RegisterFunctionInternal(internal::fsCmdBlockFinishCmd);
   RegisterFunctionInternal(internal::fsCmdBlockFinishMountCmd);
   RegisterFunctionInternal(internal::fsCmdBlockFinishReadCmd);
   RegisterFunctionInternal(internal::fsCmdBlockFinishWriteCmd);
   RegisterFunctionInternal(internal::fsCmdBlockFinishGetMountSourceNextOpenCmd);
   RegisterFunctionInternal(internal::fsCmdBlockFinishGetMountSourceNextReadCmd);
   RegisterFunctionInternal(internal::fsCmdBlockFinishGetMountSourceNextCloseCmd);
}
