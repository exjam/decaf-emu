...
getMcpData()
{
   return Library::getStaticData()->mcpData;
}
...
mcpInit()
{
   auto mcpData = getMcpData();
   mcpData->smallMessagePool = IPCBufPoolCreate(virt_addrof(mcpData->smallMessageBuffer),
                                                static_cast<uint32_t>(mcpData->smallMessageBuffer.size()),
                                                SmallMessageSize,
                                                virt_addrof(mcpData->smallMessageCount),
                                                1);

   mcpData->largeMessagePool = IPCBufPoolCreate(virt_addrof(mcpData->largeMessageBuffer),
                                                static_cast<uint32_t>(mcpData->largeMessageBuffer.size()),
                                                LargeMessageSize,
                                                virt_addrof(mcpData->largeMessageCount),
                                                1);

   mcpData->initialised = true;
}
...
mcpAllocateMessage(uint32_t size)
{
   ...
   if (size == 0) {
      ...
      message = IPCBufPoolAllocate(mcpData->smallMessagePool, size);
   } else {
      message = IPCBufPoolAllocate(mcpData->largeMessagePool, size);
   }
...
mcpFreeMessage(virt_ptr<void> message)
{
   ...
   if (IPCBufPoolFree(mcpData->smallMessagePool, message) == IOSError::OK) {
      return MCPError::OK;
   }

   if (IPCBufPoolFree(mcpData->largeMessagePool, message) == IOSError::OK) {
      return MCPError::OK;
   }
