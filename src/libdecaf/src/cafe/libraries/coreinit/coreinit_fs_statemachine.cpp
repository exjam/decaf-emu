...
{
   be2_val<AlarmCallbackFn> fsmAlarmCallback = nullptr;
};

static virt_ptr<StaticFsStateMachineData>
getFsStateMachineData()
{
   return Library::getStaticData()->fsStateMachineData;
}
...
fsmStartAlarm(virt_ptr<FSClientBody> clientBody)
{
   ...
   OSSetAlarm(virt_addrof(clientBody->fsmAlarm),
              ...
              getFsStateMachineData()->fsmAlarmCallback);
}
...
void
Library::initialiseFsStateMachineStaticData()
{
   auto fsStateMachineData = allocStaticData<StaticFsStateMachineData>();
   getStaticData()->fsStateMachineData = fsStateMachineData;

   fsStateMachineData->fsmAlarmCallback = GetInternalFunctionAddress(internal::fsmAlarmHandler);
}

void
Library::registerFsStatemachineFunctions()
{
   RegisterFunctionExport(FSGetStateChangeInfo);
   RegisterFunctionExport(FSSetStateChangeNotification);

   RegisterFunctionInternal(internal::fsmAlarmHandler);
}
