                 ...
                 uint32_t unk0x0c)
{
   ...
   *outNumMessages = numMessages;
   ...
   // Initialise FIFO.
   internal::ipcBufPoolFifoInit(virt_addrof(pool->fifo), numMessages, messageIndex);
                        ...
                        virt_ptr<IPCBufPoolAttributes> attribs)
{
   ...
   attribs->numMessages = pool->fifocount;
   OSUnlockMutex(virt_addrof(pool->mutex));
   return IOSError::OK;
}
