                        ...
                        std::vector<uint8_t> &relocations)
{
   ...
   auto addRelocation =
      [&relocations](int32_t srcOffset, uint32_t dstOffset, uint32_t symbol)
      {
         auto rela = rpl::Rela { };
   ...
   auto addTextRelocation =
      [&addRelocation](uint32_t srcOffset, uint32_t dstOffset)
      {
         addRelocation(srcOffset, dstOffset, 1);
      };
   ...
   auto addDataRelocation =
      [&addRelocation](uint32_t srcOffset, uint32_t dstOffset)
      {
         addRelocation(srcOffset, dstOffset, 2);
      };
...
Library::generateRpl()
{
   ...
   for (auto &pair : mSymbolMap) {
      ...
      if (symbol->type == LibrarySymbol::Function) {
         auto funcSymbol = reinterpret_cast<LibraryFunction *>(symbol.get());
         ...
         auto dataSymbol = reinterpret_cast<LibraryData *>(symbol.get());
   ...
   if (textSectionIndex) {
      ...
      textSection->data.resize(textSection->header.size);
   }
   ...
   if (fexportSectionIndex) {
      ...
      loadAddr += fexportSection->header.size;

      auto fexport = reinterpret_cast<be2_val<uint32_t> *>(fexportSection->data.data());
      *(fexport + 0) = numCodeExports;
      *(fexport + 1) = 0u;
   }
      ...
      for (auto &symbol : funcSymbols) {
         ...
         *(thunk + 1) = bclr.value;
         ...
         if (symbol->exported) {
            ...
            // Write to .fexport
            auto fexport = reinterpret_cast<rpl::Export *>(fexportSection->data.data() + exportOffset);
            ...
            auto rela = reinterpret_cast<rpl::Rela *>(fexportRelaSection->data.data() + relaOffset);
   ...
   if (dataSectionIndex) {
      ...
      dataSection->header.offset = 0u; // Set later
      dataSection->header.size = 0u; // Set later
   ...
   if (dexportSectionIndex) {
      ...
      loadAddr += dexportSection->header.size;

      auto dexport = reinterpret_cast<be2_val<uint32_t> *>(dexportSection->data.data());
      *(dexport + 0) = numDataExports;
      *(dexport + 1) = 0u;
   }
   ...
   if (dataSymbolsSize) {
      ...
      for (auto &symbol : dataSymbols) {
         if (symbol->exported) {
            ...
            // Write to .dexport
            auto dexport = reinterpret_cast<rpl::Export *>(dexportSection->data.data() + exportOffset);
            ...
            auto rela = reinterpret_cast<rpl::Rela *>(dexportRelaSection->data.data() + relaOffset);
            rela->info = rpl::R_PPC_ADDR32 | (symbol->index << 8);
            rela->addend = 0;
   ...
   // Generate symtab
   auto symbolCount = funcSymbols.size() + dataSymbols.size() + 1;
   symTabSection->data.resize(sizeof(rpl::Symbol) * symbolCount);
   std::memset(symTabSection->data.data(), 0, sizeof(rpl::Symbol));

   auto getSymbol =
      [&symTabSection](uint32_t index)
      {
         return reinterpret_cast<rpl::Symbol *>(symTabSection->data.data() + (1 * sizeof(rpl::Symbol)));
      };
   ...
   loadAddr += symTabSection->header.size;
   ...
   loadAddr += strTabSection->header.size;
   ...
   loadAddr += shStrTabSection->header.size;
   ...
   for (auto i = 0u; i < sections.size(); ++i) {
      ...
      *reinterpret_cast<be2_val<uint32_t> *>(crcSection->data.data() + i * 4) = crc;
   }
   ...
   offset += align_up(static_cast<uint32_t>(sizeof(rpl::SectionHeader) * sections.size()), 64);
   ...
   for (auto &section : sections) {
      std::memcpy(mGeneratedRpl.data() + offset, &section.header, sizeof(rpl::SectionHeader));
      offset += sizeof(rpl::SectionHeader);
   }

   for (auto &section : sections) {
      if (section.header.offset && section.data.size()) {
         std::memcpy(mGeneratedRpl.data() + section.header.offset, section.data.data(), section.data.size());
      }
   }
}
