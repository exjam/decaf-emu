...
struct TempSyncEvent
{
   be2_struct<OSEvent> event;
...
struct TempDirData
{
   ...
   be2_array<TempSyncEvent, NumSyncEvents> syncEventList;
   be2_struct<OSCondition> syncEventListCond;
   be2_struct<OSMutex> syncEventListMutex;
   be2_val<uint32_t> syncEventListFreeBitmask = (1 << syncEventList.size()) - 1;
};
...
static virt_ptr<TempSyncEvent>
acquireSyncEvent()
{
   auto event = virt_ptr<TempSyncEvent> { nullptr };
...
static void
releaseSyncEvent(virt_ptr<TempSyncEvent> event)
{
   OSLockMutex(virt_addrof(sTempDirData->syncEventListMutex));
   for (auto i = 0u; i < sTempDirData->syncEventList.size(); ++i) {
...
{
   auto syncEvent = virt_cast<TempSyncEvent *>(context);
   syncEvent->result = static_cast<TEMPStatus>(status);
   OSSignalEvent(virt_addrof(syncEvent->event));
}
                          ...
                          virt_ptr<const FSAsyncData> asyncData)
{
   ...
   auto error = TEMPGetDirPath(dirId, virt_addrof(sTempDirData->dirPath), sTempDirData->dirPath.size());
   if (error == TEMPStatus::OK) {
