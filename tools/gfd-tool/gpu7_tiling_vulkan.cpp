...
#include "bin2c_vulkan_shaders.h"
...
private:
   ...
   */
   VkBuffer mSrcBuffer;
   VkDeviceMemory mSrcBufferMemory;

   VkBuffer mDstBuffer;
   VkDeviceMemory mDstBufferMemory;
...
public:
   void run(const SurfaceInfo &surface, void *src, void *dst)
   {
      // Buffer size of the storage buffer that will contain the rendered mandelbrot set.
      mWidth = gpu7::tiling::calculateAlignedPitch(surface);
      mHeight = gpu7::tiling::calculateAlignedHeight(surface);
      mDepth = gpu7::tiling::calculateAlignedDepth(surface);
      mBufferSize = (surface.bpp / 8) * mWidth * mHeight * mDepth;
      ...
      createDescriptorSet();
      createComputePipeline(surface);
      createCommandBuffer(surface);
      ...
      saveRenderedImage(dst);

      // Clean up all vulkan resources.
      cleanup();
   }

   void saveRenderedImage(void *dst)
   {
      void* mappedMemory = NULL;

      VkMappedMemoryRange range[1] = { { } };
      range[0].sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
      range[0].memory = mDstBufferMemory;
      range[0].size = mBufferSize;
      range[0].offset = 0;
      VK_CHECK_RESULT(vkInvalidateMappedMemoryRanges(device, 1, range));

      vkMapMemory(device, mDstBufferMemory, 0, mBufferSize, 0, &mappedMemory);
      std::memcpy(dst, mappedMemory, mBufferSize);
      vkUnmapMemory(device, mDstBufferMemory);
   }
   ...
   void createSrcBuffer(void *src)
   {
      ...
      */
      ...
      bufferCreateInfo.size = mBufferSize; // buffer size in bytes.
      bufferCreateInfo.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT; // buffer is used as a storage buffer.
      bufferCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // buffer is exclusive to a single queue family at a time.

      VK_CHECK_RESULT(vkCreateBuffer(device, &bufferCreateInfo, NULL, &mSrcBuffer)); // create buffer.
      ...
      */
      ...
      vkGetBufferMemoryRequirements(device, mSrcBuffer, &memoryRequirements);
      ...
      */
      ...
      VK_CHECK_RESULT(vkAllocateMemory(device, &allocateInfo, NULL, &mSrcBufferMemory)); // allocate memory on device.
      // TODO: Upload here?

      // Now associate that allocated memory with the buffer. With that, the buffer is backed by actual memory.
      VK_CHECK_RESULT(vkBindBufferMemory(device, mSrcBuffer, mSrcBufferMemory, 0));

      void* mappedMemory = NULL;
      VK_CHECK_RESULT(vkMapMemory(device, mSrcBufferMemory, 0, mBufferSize, 0, &mappedMemory));
      std::memcpy(mappedMemory, src, mBufferSize);

      VkMappedMemoryRange range[1] = {};
      range[0].sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
      range[0].memory = mSrcBufferMemory;
      range[0].size = mBufferSize;
      VK_CHECK_RESULT(vkFlushMappedMemoryRanges(device, 1, range));

      vkUnmapMemory(device, mSrcBufferMemory);
   }

   void createDstBuffer()
   {
      /*
      We will now create a buffer. We will render the mandelbrot set into this buffer
      in a computer shade later.
      */
      ...
      VK_CHECK_RESULT(vkCreateBuffer(device, &bufferCreateInfo, NULL, &mDstBuffer)); // create buffer.
      ...
      */
      VkMemoryRequirements memoryRequirements;
      vkGetBufferMemoryRequirements(device, mDstBuffer, &memoryRequirements);
      ...
      */
      ...
      VK_CHECK_RESULT(vkAllocateMemory(device, &allocateInfo, NULL, &mDstBufferMemory)); // allocate memory on device.

      // Now associate that allocated memory with the buffer. With that, the buffer is backed by actual memory.
      VK_CHECK_RESULT(vkBindBufferMemory(device, mDstBuffer, mDstBufferMemory, 0));
   }
   ...
   void createDescriptorSet()
   {
      ...
      */
      ...
      descriptorBufferInfo[0].buffer = mSrcBuffer;
      descriptorBufferInfo[0].offset = 0;
      descriptorBufferInfo[0].range = mBufferSize;

      descriptorBufferInfo[1].buffer = mDstBuffer;
      descriptorBufferInfo[1].offset = 0;
      descriptorBufferInfo[1].range = mBufferSize;
   ...
   struct MicroTilePushConstants
   {
      ...
      uint32_t sliceOffset; // TODO: Calculate sliceOffset in shader so we can do slices group.z
      uint32_t bankSliceRotation;
      uint32_t sampleSliceRotation;
      uint32_t pipeSliceRotation;
      uint32_t bankSwapInterval;
      uint32_t macroTileUnits;
      uint32_t microTileUnits;
   ...
   void createComputePipeline(const SurfaceInfo &surfaceInfo)
   {
      /*
      ...
      */
      ...
      createInfo.pCode = (const uint32_t *)bin2c_gpu7_tiling_comp_spv;
      createInfo.codeSize = bin2c_gpu7_tiling_comp_spv_size;
      ...
      */
      ...
      TileShaderSpecialisation spec_values = { };
      spec_values.bpp = surfaceInfo.bpp;
      spec_values.macroTileWidth = getMacroTileWidth(surfaceInfo.tileMode);
      spec_values.macroTileHeight = getMacroTileHeight(surfaceInfo.tileMode);
      VkSpecializationMapEntry entry[] = {
         { 0, offsetof(TileShaderSpecialisation, bpp), sizeof(uint32_t) },
         { 1, offsetof(TileShaderSpecialisation, macroTileWidth), sizeof(uint32_t) },
         { 2, offsetof(TileShaderSpecialisation, macroTileHeight), sizeof(uint32_t) },
      };
      VkSpecializationInfo spec_info ={
         sizeof(entry) / sizeof(entry[0]), entry,
         sizeof(TileShaderSpecialisation),
         &spec_values
      };
      ...
      */
      ...
      push_constants[0].size = sizeof(MicroTilePushConstants);
   ...
   void createCommandBuffer(const SurfaceInfo &info)
   {
      /*
      ...
      */
      ...
      const auto microTileThickness = gpu7::tiling::getMicroTileThickness(info.tileMode);
      const auto microTileBytes =
         MicroTileWidth * MicroTileHeight * microTileThickness
         * bytesPerElement; // *surface.numSamples; fuck you sample tiling
      const  auto sliceBytes =
         mWidth * mHeight * microTileThickness * bytesPerElement;

      MicroTilePushConstants pushConstants;
      pushConstants.dstStride = mWidth * (info.bpp / 8);
      pushConstants.microTileBytes = microTileBytes;
      pushConstants.sliceBytes = sliceBytes;
      vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(MicroTilePushConstants), &pushConstants);
      ...
      */
      vkCmdDispatch(commandBuffer, mWidth / MicroTileWidth, mHeight / MicroTileWidth, mDepth);
   ...
   void cleanup()
   {
      ...
      */
      ...
      vkFreeMemory(device, mSrcBufferMemory, NULL);
      vkFreeMemory(device, mDstBufferMemory, NULL);
      vkDestroyBuffer(device, mSrcBuffer, NULL);
      vkDestroyBuffer(device, mDstBuffer, NULL);
...
untileImage(const SurfaceInfo &surface,
            ...
            void *dst)
{
   ComputeApplication a;
   a.run(surface, src, dst);
}
